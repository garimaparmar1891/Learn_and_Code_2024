...
class CartService:
    ...
    def add_to_cart(user_id, product_name, quantity):
        ...
        existing_item = CartModel.get_cart_items(user_id)

        if any(item[0] == product_name for item in existing_item):
            success = CartModel.update_cart_quantity(user_id, product_id, quantity)
        else:
